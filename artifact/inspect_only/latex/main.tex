...
The issue-level package probe separates visible-passing edits: the main lane reports three pass and one fail, and the Qwen lane reports one pass and four fail, with the passing group constructing the 4-by-4 axes grid shown in Figure~\ref{fig:probe-code-cards}.
The certificate is formed from public behavior alone, so the case supports independent-source compatibility without carrying a broad Qwen reproduction claim.
...
In the Qwen lane, issue-specific probes remain all-fail, while a package-integrity probe reports two pass and three fail on whether LabelEncoder can be imported and constructed.
\patchcourt records both facts but treats the integrity split as package-risk evidence rather than issue-correctness evidence.
...
This cheap issue-tied layer is stronger than a source-isolated template while remaining separate from held-out calibration.
