...
\begin{abstract}
Repository-level LLM repair systems routinely stop when a candidate applies cleanly and visible tests pass.
In issue-driven benchmarks, this stopping rule can leave an important question unresolved: several visible-passing patches may satisfy the same public tests while disagreeing on the behavior implied by the issue.
\patchcourt audits this acceptance layer.
It treats visible-passing patches as executable hypotheses, compares them with layered public probes, records behavioral-disagreement certificates, and freezes the certificate index before held-out labels are opened.
Post-freeze calibration then measures how often these accepted-looking records fail held-out issue tests.
In a 50-task pilot, 200 candidate slots produce 68 visible-passing candidates and 37 raw certificates; conservative filtering yields 30 task-level audit records, all executable under held-out calibration.
Within this selected certificate-producing path, 17/30 probe-accepted records become false acceptances and 26/30 retain specification-debt signals.
...
A local Qwen stress lane adds one issue-specific and four package-integrity disagreement cases.
The artifact regenerates the headline tables and links each count to certificate records.
The results argue for shifting visible tests from a final repair endpoint to a pre-acceptance review trigger.
\end{abstract}
...
Visible tests are an attractive stopping rule.
...
The practical temptation is clear: if the patch applies and the visible commands pass, accept it.
...
Visible success therefore leaves a question that is easy to miss during repair: did the acceptance step actually collapse the candidate space to behaviorally compatible fixes?
...
No developer patch is needed to observe the split.
The resulting certificate is deliberately modest: it records that visible tests admitted behaviorally incompatible hypotheses.

\emph{False acceptance} names the downstream failure of this acceptance step.
\patchcourt studies it after candidate patches exist and before held-out labels are read.
Its output is executable evidence that a visible-pass pool still contains unresolved behavior and should be inspected, reranked, or held back.
...
The study uses a frozen 30-certificate audit set drawn from SWE-bench-style repository repair tasks after visible-pass and pre-freeze public-probe evidence are available.
The estimand is therefore conditional on a certificate-producing path: among auditable tasks that already expose conservative pre-freeze behavioral disagreement, how often does visible/probe acceptance fail held-out calibration?
...
The same evidence also fixes the interpretation: the paper measures a high-risk, auditable acceptance regime; raw disagreement scores remain under-calibrated; and the Qwen lane is used as a stress test of candidate-source compatibility.

The contributions are:
\begin{itemize}[leftmargin=*]
\item a characterization of visible-test false acceptance within a selected certificate-producing path under frozen, held-out calibration;
\item the \patchcourt protocol, which records executable disagreement evidence before evaluation labels are read;
\item a layered probe evaluation that separates source-isolated templates, issue-code probes, full-package issue probes, package-integrity checks, and held-out calibration;
\item a triage and candidate-source stress analysis, including a local Qwen lane with low issue-specific but nonzero disagreement yield.
\end{itemize}
...
The probe is a public, issue-facing measurement of the reported edge case.
...
\begin{figure}[t]
  ...
  \Description{A Matplotlib public probe outcome chart: four visible-passing candidates enter, three pass the public probe, one fails it, and the certificate is marked as a pass/fail behavior split.}
...
The frozen path uses an interactive model-generated lane and a project-controlled batch lane; both keep candidate provenance so every certificate can be traced back to its source.
The separate stress lane uses a locally served Qwen2.5-Coder model with structured search/replace generation.
Table~\ref{tab:candidate-provenance} records how each lane enters the claims.
...
\toprule
Lane & Frozen & Candidate evidence & Use in claims \\
\midrule
Interactive model-generated lane & 7 & Model patch evidence; thirteen certificates before conservative deduplication. & Part of the selected certificate-producing path. \\
Project-controlled batch lane & 23 & Additional calibration diversity with source provenance preserved in the artifact. & Main selected-path source, reported as project-controlled candidate evidence. \\
...
Package-integrity splits describe import or API regression risk; issue behavior is measured with issue-code, full-package issue probes, or held-out calibration.
...
Post-disagreement overlays strengthen execution on already identified public behavior surfaces and are reported as selected upgrade evidence.
...
\midrule
Source-isolated & Post-issue and pre-held-out; often after candidate-context materialization, before calibration labels. & Cheap issue-facing discovery signal; it flags specification debt before package execution is available. \\
Issue-code & Public snippet exists pre-candidate; adaptation/routing may be post-disagreement, still gold-blind. & Issue-tied execution evidence for selected records. \\
Full-package & Mostly post-disagreement audit-upgrade overlay for selected targets, before label reading. & Robustness check for selected certificates under repository package execution. \\
...
evidence: one issue-specific Qwen BDC
\end{lstlisting}
\end{minipage}
...
The audit saw probe layer and candidate source because those fields determine what each certificate can support; held-out outcomes remained hidden until the index was frozen.
The result is a transparent single-pass audit; inter-rater reliability remains a validity threat.
The artifact exposes stable certificate links, pass/fail behavior, audit rationales, downgrade reasons, and selected calibration candidates.
...
The frozen input had already filtered to conservative pass/fail records, so no record in the frozen 30 met the reject criteria; weaker surviving cases were marked as downgrades.
The single-pass audit is made inspectable through examples: retained Matplotlib and Seaborn records expose issue/full-package pass/fail behavior on visible-pass candidates, while downgraded Matplotlib and Sphinx records remain in the index with source-isolated, routing, or missing-upgrade caveats.
...
\ccogb is reported as a lower-bound risk signal.
...
Wilson intervals are reported for $\widehat{\theta}_S$, the selected-path estimand.
...
\textbf{RQ4 (independent-source stress test).} What changes when candidate patches come from a locally served Qwen model?
...
This design makes each row checkable enough for an ICSE artifact review and keeps the rate tied to the selected certificate-producing path.
...
Independent-source gate & Qwen stress lane only & Batch candidates remain project-controlled evidence. \\
\bottomrule
\end{tabularx}
\end{table}

\begin{table}[t]
\caption{Cost and overhead summary for the paper-facing layers. The table reports work units and execution counts.}
\label{tab:overhead}
\centering
\scriptsize
...
Ranking and correlation results are reported as preliminary triage evidence.
...
\begin{table}[t]
\caption{Frozen held-out calibration after the certificate index is frozen.}
\label{tab:heldout}
\centering
\begin{tabular}{lr}
\toprule
...
On this path, visible/probe acceptance is a risky endpoint unless the residual public behavior is inspected.
...
The upgraded subset is small, so pass/fail BDCs and all-fail divergences are reported as separate evidence types.
...
These values remain inside the finite-sample random intervals around random review, so the signal is best read as a review cue at this sample size.
...
The rule is broad, but it shows that the ledger can drive explicit risk-control decisions.
...
The independent-source experiment asks how far the evidence pipeline carries when candidate patches come from a non-Codex local model source.
...
This lane is a partial-positive stress test: \patchcourt ingests a real non-Codex source and finds nonzero disagreement, while the observed issue-specific yield remains low.
...
\begin{table*}[t]
\caption{Evidence reading by result type.}
\label{tab:claim-boundary}
\centering
\begin{tabularx}{\textwidth}{lXX}
\toprule
Evidence type & Supported inference & Reading condition \\
\midrule
Frozen held-out calibration (17/30 false acceptance; 26/30 spec debt) & Visible/probe acceptance often fails held-out labels in the selected certificate-producing path. & Conditional on the frozen selected path. \\
Main and extra probe upgrades (6/10 and 6/10 pass/fail) & Stronger full-package or issue-code probes preserve disagreement for selected targets. & Evidence for selected upgrade targets. \\
Qwen issue-specific pass/fail BDC (1 task) & A single independent-source issue-specific compatibility case; strongest case is Seaborn MultiIndex pairplot. & Low observed issue-specific yield. \\
Qwen package-integrity BDCs (4 tasks) & Independent-source visible-pass patches can break or preserve full-package behavior differently. & Integrity/regression reading. \\
Qwen all-fail divergences and rescue negatives & Diagnostic behavior diversity and a documented negative result. & Negative applicability evidence. \\
\bottomrule
\end{tabularx}
\end{table*}

\noindent\textbf{Qwen stress lane.}
...
Raw generation and applicability counts are preserved in the artifact as diagnostic evidence.
...
Panel D shows the reranking result: the best review cue improves over raw \ccogb, but the finite-sample random baseline remains close.
...
The following cases put the aggregate tables back into certificate-level behavior.
Each case links a table-level result to an inspectable audit row.
...
In the Qwen lane, issue-specific probes remain all-fail, while a package-integrity probe reports two pass and three fail on whether LabelEncoder can be imported and constructed; \patchcourt records that split as package-risk evidence.
...
evidence: semantic split before held-out labels
\end{lstlisting}
  ...
  \Description{A compact code-like Django probe card showing render_join with autoescape disabled, three passing candidates preserving alpha newline beta, one failing candidate with an escaped separator assertion, and an evidence note that the semantic split is recorded before held-out labels.}
...
\patchcourt takes a prospective route: it freezes an evidence ledger before developer patches, hidden tests, or stronger labels enter the process, then uses held-out calibration to measure alignment with later failure.
The payoff is an acceptance-time signal that can be inspected before benchmark answers are opened.
...
\patchcourt treats probes as evidence-producing instruments whose strength depends on provenance and execution layer.
...
All-fail divergence is weaker than pass/fail disagreement, and package-integrity probes measure regression or API preservation; the tables keep those evidence types separate from issue behavior.
...
The triage results are an early review-prioritization signal, with predictor validation left to larger sets.

\textbf{Independent-source validity.}
One issue-specific pass/fail BDC and four package-integrity BDCs make the Qwen lane a stress test with low issue-specific yield.
...
\patchcourt's main design invariant is the pre-freeze/post-freeze separation.
The artifact must keep this separation inspectable.
Any future experiment that consults developer patches, test patches, FAIL\_TO\_PASS, or PASS\_TO\_PASS before freeze should be reported as a separate retrospective analysis.
...
When public executable evidence still shows unresolved behavior, visible tests should trigger inspection, reranking, or abstention before a repository-level repair is accepted.
