...
Figure~\ref{fig:structured-patch-record} shows the normalized edit-record interface used for deterministic application, route logging, and later certificate linkage.
...
[
  {
    ...
    "insertions": [
      {"line_number": 2418,
       "new_lines": [
         "# ... issue-specific guard ...\n",
         "if _all_nan_bar_input(height):\n",
...
\end{lstlisting}
  \caption{Example normalized patch record consumed before visible-test and probe execution; shortened for display.}
  \Description{JSON-like patch record with a task id, file path, insertions, deletions, modifications, and pre-freeze evidence placeholders.}
